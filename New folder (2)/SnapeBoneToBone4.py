...
class myProperties_PG(bpy.types.PropertyGroup):
    sourceArmature:bpy.props.StringProperty(name="sourceArmature",default="")
...
class pastAnimation_OP(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        armatureSource = bpy.data.objects[myProp.sourceArmature]
        armatureTarget = bpy.data.objects[myProp.targetArmature]
        ...
        if animationRange:            
            for i in range(animationRange['min'],animationRange['max']):
             ...
             self.report({'WARNINE'}, "Source Armature haven't got keyframes")
        return{'FINISHED'}
